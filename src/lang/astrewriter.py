...
class ASTRewriter:
    ...
    def set_node_attr(self, attr, value=True):
        nodeattrs.set_attr(self.node.get(), attr, value)
        return self
    ...
    def append_to_body(self, *nodes):
        ...
        for n in nodes_to_add:
            ...
            if ti is None:
                nodeattrs.set_type_info(n, tim.TypeInfo.notype())
        ...
        if util.types.instanceof_py_function(function):
            ...
            if rtn_type is not None:
                if isinstance(rtn_type, tim.TypeInfo):
                    ...
                    rtn_type_info = tim.TypeInfo(rtn_type)
    ...
    def xcall(self, function):
        ...
        """
        return self.call(function, tim.TypeInfo.notype())
    ...
    def xident(self, name_or_node, node_attrs=[]):
        ...
        if isinstance(name_or_node, str):
            return self.ident(name_or_node, tim.TypeInfo.notype(), node_attrs)
        elif isinstance(name_or_node, ast.Name):
            return self._ident(name_or_node, tim.TypeInfo.notype(), node_attrs)
        else:
    ...
    def _ident(self, name_node, type_info=None, node_attrs=[]):
        ...
        if type_info is not None:
            if not isinstance(type_info, tim.TypeInfo):
                type_info = tim.TypeInfo(type_info)
    ...
    def less_than(self, node_attrs=[]):
        ...
        self.ast_context.register_type_info_by_node(n, tim.TypeInfo.bool())
        nodeattrs.set_type_info(n, tim.TypeInfo.bool())
        return ASTRewriter(n, arg_nodes=[], ast_context=self.ast_context,
                           parent_body=self.parent_body)
    ...
    def reassign_to_arg(self):
        ...
        rtn_type = tim.TypeInfo.late_resolver(lambda ati: ati)
